...
Validated on `main` at `0db5b38` (#60, 2026-09-03). Gates run there: 759
tests passed across 558 SDK in 49 files, 9 P0, and 192 demo; the evaluation
harness's own 96 tests passed; all three TypeScript projects typechecked;
SDK and both applications built; the packed SDK imported and executed under
plain Node; and the design doc check passed at 119 claims and 228 anchors.
Every finding below is resolved on that commit.

The findings were reviewed on branch commits `6a1745e`, `812e5b9`,
`2f1f332`, `2bc6f6a`, `d7a4911`, `0c4f2fa`, `798c899`, `0123fbc`,
`2f1f9e8`, `99d4139`, `253a3be`, and `78651be`. Eleven of the twelve are
not reachable from `main`, because their pull requests were squash-merged.
They are recorded here so the findings stay resolvable after the branches
are pruned.
...
| `2bc6f6a` | no | `dd60ec4` (#8, from `fix/acting-identity`) |
| `d7a4911` | no | `dd60ec4` (#8, from `fix/acting-identity`) |
| `0c4f2fa` | no | `3d1f904` (#11, from `fix/staged-proposals`) |
| `798c899` | no | `3d1f904` (#11) |
| `0123fbc` | no | `3d1f904` (#11) |
| `2f1f9e8` | no | `87e6d6e` (#14, from `feat/agentdesk-evals`) |
| `99d4139` | no | `87e6d6e` (#14) |
| `253a3be` | no | `87e6d6e` (#14) |
| `78651be` | no | `87e6d6e` (#14) |

The six acting-identity findings landed on `main` as #8 (`dd60ec4`). Their
reproductions live on the branch commits above: the first at `32b99db`,
four `2026-08-30` findings on `ef68151`, and the two identity-parsing
findings on `d7a4911` through `packages/webmcp/tests/actor-parsing.test.ts`.
The wave 1 durability work that resolves the accepted-risk record below is
#46, landed on `main` by #52 (`f7835ea`).
...
| P1 | RESOLVED (#9) | [A rollback that commits and throws can run twice](2026-08-29-p1-throwing-rollback-can-run-twice.md) |
| P2 | RESOLVED (#9) | [Repeated live-region announcements are dropped](2026-08-29-p2-identical-live-announcements-are-dropped.md) |
...
| P1 | RESOLVED (#8) | [Human identity shape is not validated at the JavaScript boundary](2026-08-30-p1-human-identity-shape-is-not-validated.md) |
...
| P1 | RESOLVED (#9, after #10) | [Stop strands an interrupted rollback in progress](2026-08-30-p1-stop-strands-rollback-in-progress.md) |
...
| P2 | RESOLVED (on `main`, `receipts.ts` and `audit.ts` type the reconciler as `HumanActor`) | [Reconciler fields remain typed as generic actors](2026-08-31-p2-reconciler-fields-remain-generic-actors.md) |
...
| — | RESOLVED (#46, landed by #52) | [Unreconciled records and artifacts do not survive a restart](2026-08-31-accepted-unreconciled-records-are-not-durable.md) |
