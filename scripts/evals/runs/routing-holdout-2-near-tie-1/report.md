...
Run `routing-holdout-2-near-tie-1` at 2026-09-03T01:42:15.638Z.
...
| Metric | Deterministic (lexical, the shipped default) | Custom scorer (hierarchical-near-tie-1.0, external) | Provenance |
| --- | --- | --- | --- |
| Expected capability in the routed set | 29.1% | 36.4% | measured |
| Rank of the expected capability, when routed (mean) | 1.88 | 1.90 | measured |
| Routed set size (mean) | 3.93 | 4.09 | measured |
| Schema bytes the routed set registers (mean) | 1,152 | 1,208 | measured |
| Tie at the cut | 61.8% | 52.7% | measured |
| Prompt overlap with the expected metadata (mean) | 0.10 | 0.10 | measured |
...
### Custom scorer (hierarchical-near-tie-1.0, external)
...
| `c2-release` | Acme paid what they owed, lift the freeze on their account | `release_customer` | none in top six | `accounts_hold_account`, `hold_account`, `accounts_get_account`, `accounts_release_account`, `accounts_update_account` |
| `c2-address-find` | Which of our customers are at 12 Harbour Street | `find_address` | none in top six | `archive_customer`, `create_customer`, `create_customer_tag`, `delete_customer`, `delete_customer_note` |
| `a2-new-user` | Get Priya's replacement set up with a login before Monday | `create_user` | none in top six | `get_return_label`, `returns_get_order`, `returns_get_refund`, `approve_exchange`, `cancel_exchange` |
| `a2-sessions` | Where is Jonas logged in right now | `list_session` | none in top six | nothing |
| `v2-transfer-approve` | The 40 units heading to Cork can go, I've checked the paperwork | `approve_stock_transfer` | none in top six | nothing |
...
| `k2-variant-new` | We're adding an XXL in the blue hoodie | `create_variant` | none in top six | nothing |
| `k2-category-merge` | Outerwear and Jackets are the same shelf online, combine them | `merge_category` | none in top six | `assign_bin`, `get_bin`, `list_bin`, `update_bin` |
| `r2-exchange-new` | Ravi wants a medium instead of the large he got | `create_exchange` | none in top six | nothing |
...
| `r2-hold-return` | Hold Ravi's return until the courier confirms they collected it | `hold_return` | none in top six | `hold_delivery`, `shipping_hold_shipment`, `archive_carrier`, `assign_carrier`, `create_carrier` |
...
| `p2-report-hold` | Don't publish the Q3 numbers yet, the CFO hasn't signed them off | `hold_report` | none in top six | `create_tracking_number`, `find_tracking_number`, `get_tracking_number`, `cancel_delivery`, `shipping_cancel_shipment` |
